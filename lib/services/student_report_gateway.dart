/*
 * 学工报表 HTTP 网关 — 手动维护 OA/CAS Cookie 与跳转链路
 * @Project : SSPU-AllinOne
 * @File : student_report_gateway.dart
 * @Author : Qintsg
 * @Date : 2026-05-01
 */
...
class DioStudentReportGateway implements StudentReportGateway {
  ...
  final Dio _dio;
  final _StudentReportCookieStore _cookieStore = _StudentReportCookieStore();
  ...
  Future<StudentReportHttpSnapshot> openEntryPage(
    ...
    return _sendWithRedirects(uri: entranceUri, timeout: timeout);
  }

  @override
  Future<StudentReportHttpSnapshot> fetchPage(Uri pageUri, Duration timeout) {
    return _sendWithRedirects(uri: pageUri, timeout: timeout);
  }

  Future<StudentReportHttpSnapshot> _sendWithRedirects({
    required Uri uri,
...
class _StudentReportCookieStore {
