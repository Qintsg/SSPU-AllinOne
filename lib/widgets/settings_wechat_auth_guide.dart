...
class SettingsWechatAuthGuide extends StatelessWidget {
  ...
  Widget _buildFaq(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('FAQ', style: textTheme.titleSmall),
        const SizedBox(height: AppSpacing.xs),
        Text('Q：个人一定要完成额外认证才能用吗？', style: textTheme.titleSmall),
        Text(
          'A：本应用需要的是“你能正常登录公众平台后台并拿到登录态”。是否还需要做后续认证，取决于你自己的运营需求和平台当时规则。',
          style: textTheme.bodyMedium,
        ),
        const SizedBox(height: AppSpacing.xs),
        Text('Q：为什么这一方式要先注册公众号？', style: textTheme.titleSmall),
        Text(
          'A：因为应用调用的是公众平台后台接口，必须先有一个能登录后台的账号作为入口。',
          style: textTheme.bodyMedium,
        ),
        const SizedBox(height: AppSpacing.xs),
        Text('Q：为什么现在只保留这一种方式？', style: textTheme.titleSmall),
        Text(
