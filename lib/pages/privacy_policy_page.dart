...
1. 桌面端应用数据默认保存在 `~/.sspu-all-in-one/`；Android / iOS 保存在系统分配的应用支持目录；Web 端使用浏览器提供的本地存储能力。
...
5. 若需要手动重建本地状态，可在退出应用后备份并删除 `~/.sspu-all-in-one/` 或移动端系统应用数据目录中的相关文件。系统安全存储中的凭据建议优先通过应用内入口清除。
