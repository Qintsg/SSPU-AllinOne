...
请在使用本软件前仔细阅读本协议。本协议说明 SSPU-AllinOne 如何在您的设备上处理数据、访问外部服务，以及您可以如何管理本地数据。

一、适用范围

1. 本协议适用于 SSPU-AllinOne 应用本身。
...
1. 桌面端应用数据默认保存在 `~/.sspu-all-in/`；Android / iOS 保存在系统分配的应用支持目录；Web 端使用浏览器提供的本地存储能力。
...
5. 若需要手动重建本地状态，可在退出应用后备份并删除 `~/.sspu-all-in/` 或移动端系统应用数据目录中的相关文件。系统安全存储中的凭据建议优先通过应用内入口清除。
