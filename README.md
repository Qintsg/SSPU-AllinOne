...
> 声明：本应用由于仅适配上海第二工业大学，故在项目中使用了SSPU等表述，该表述**并非**用于声明任何本项目和上海第二工业大学的合作关系，并非用于宣传等途径，不构成任何建议和参考。本项目仅为**非官方**的个人**非盈利**开源项目，且不保证后续更新。本项目目前不受任何资助。
...
- 当前 Android 应用 ID 为 `cn.qintsg.sspu_allinone`。仅改应用名不需要更换 keystore；本次包名迁移后，Android 会把新包名识别为另一款应用，既有 keystore 可继续用于签名，但不能作为旧包名应用的原地升级。
