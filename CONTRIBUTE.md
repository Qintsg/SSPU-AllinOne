...
安装完成后重启终端。不同安装方式可能提供 `gitflow` 或 `git flow`，请用 `gitflow --version` / `git flow version` 探测可用命令。
...
Linux 的命令表面取决于发行版包和 Git Flow 版本，常见为 `git flow`，也可能是 `gitflow`。

#### 初始化

本仓库通常已经完成 Git Flow 初始化。只有在全新 clone 且 `git config --get-regexp '^gitflow\.'` 没有输出时，才在仓库根目录执行。以下示例中的 `<git-flow>` 表示本机可用的 `gitflow` 或 `git flow`：
...
# 2. 从 develop 创建功能分支
<git-flow> feature start wechat-subscription
...
# 6. PR 合并后，清理本地分支
<git-flow> feature finish wechat-subscription
```
...
| `bugfix/` | Bug 修复 | `bugfix/macos-runner-config` |
...
| `deps/` | 依赖更新 | `deps/flutter-upgrade` |

支持带个人前缀的格式，例如 `qintsg/feature/wechat-subscription`、`alice/bugfix/login-bug`。

注意：Bug 修复分支前缀是 `bugfix/`，但 Commit Message 与 PR 标题的 type 仍使用 `fix(scope): 中文摘要`。
...
# 本地状态与最近提交校验（lore-protocol@0.5.0 无内置 status 时使用）
