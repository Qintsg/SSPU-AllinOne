...
- **Material 3 设计语言**：采用 Flutter Material 3 组件、主题 token 与响应式导航，提供现代、一致的视觉体验
...
| UI 体系 | Flutter Material 3 | SDK 内置 | Material 3 组件、ColorScheme、响应式导航与设计 token |
...
├─────────────────────────────────────────────────────────────┤
│                         AppShell                             │
│       Adaptive Material Navigation + Bottom Navigation       │
├─────────────┬─────────────┬─────────────┬─────────────┬─────────────┬─────┤
...
| 层级 | 目录 | 职责 |
|------|------|------|
| 入口层 | `lib/main.dart` | 平台能力初始化、MaterialApp 配置、协议确认 / 密码 / 托盘生命周期 |
| 导航层 | `lib/app.dart` | Material 3 自适应导航、移动端底部导航、页面切换容器 |
| 页面层 | `lib/pages/` | 主页、教务、信息中心、设置、关于、登录 WebView 等页面 |
| 组件层 | `lib/widgets/` | 设置分区、消息项、Material 3 surface、频道列表、响应式布局等可复用 UI |
...
### 3.1 Material 3 自适应导航结构

应用在紧凑宽度使用 `NavigationBar`，在中等与扩展宽度使用 `NavigationRail`，在大屏宽度使用 `NavigationDrawer`，保持桌面、平板与移动端的导航层级一致。

| 导航项 | 图标 | 位置 | 对应页面 |
|--------|------|------|----------|
| 主页 | `Icons.home_outlined` | 主区域 | `HomePage` |
| 教务中心 | `Icons.school_outlined` | 主区域 | `AcademicPage` |
| 课程表 | `Icons.calendar_month_outlined` | 主区域 | `CourseSchedulePage` |
| 信息中心 | `Icons.info_outline` | 主区域 | `InfoPage` |
| 学校邮箱 | `Icons.mail_outline` | 主区域 | `EmailPage` |
| 快速跳转 | `Icons.link` | 主区域 | `QuickLinksPage` |
| 设置 | `Icons.settings_outlined` | 次级区域 | `SettingsPage` |
| 关于 | `Icons.info_outline` | 次级区域 | `AboutPage` |

应用桌面 / 平板侧边导航在“设置”上方显示校园网 / VPN 状态徽标，启动后自动检测一次，点击徽标可重新检测。当前默认通过只读访问 `https://tygl.sspu.edu.cn/` 判断校园受限资源是否可达。

### 3.2 显示模式

桌面布局按 Material 3 window size class 切换导航形态，移动端使用底部导航栏：

- **Compact**（< 600px）：底部 `NavigationBar`
- **Medium**（600–840px）：紧凑 `NavigationRail`
- **Expanded**（840–1200px）：扩展 `NavigationRail`
- **Large / Extra large**（>= 1200px）：`NavigationDrawer`

### 3.3 页面切换动画

所有主页面切换使用 Material 3 导航容器的选中状态切换；手机底部导航通过 `KeyedSubtree` 强制刷新当前页，保持切换后的内容状态与动画一致。

### 3.4 Material 3 surface 与微动效

主导航页面统一使用 Material 3 surface 与共享设计 token 承载核心内容区。组件集中处理浅色 / 深色背景、描边、圆角、阴影、悬停高亮和按压缩放反馈，避免页面各自硬编码视觉状态。页面入场动画继续使用 `flutter_animate` 的淡入与轻微纵向位移，并通过共享 motion token 对多张卡片做错峰入场。
...
- `PageHeader` 显示页面标题
- Material 3 surface 组件包裹欢迎区、校园卡余额和最新消息摘要
...
**UI 结构**：说明区、学期概览和按星期分组的课程块统一使用 Material 3 surface，保持与教务中心一致的边框、圆角和阴影层级。
...
**UI 结构**：操作、刷新进度、搜索和筛选集中在同一个 Material 3 surface 中，消息列表和空状态使用独立 surface，减少分散按钮和裸列表带来的层级割裂。
...
**UI 结构**：搜索框和最佳匹配入口合并为一个操作 surface；链接磁贴使用可点击 Material 3 surface，悬停时显示强调色边框、柔和背景和阴影提升。
...
**UI 结构**：页面说明、协议操作、读取进度、邮箱摘要和邮件摘要卡片统一使用 Material 3 surface，保持与课程表、教务中心一致的阴影和边框层级。
...
在 `main.dart` 的 `MaterialApp` 中统一接入 `AppTheme.build(Brightness.light)` / `AppTheme.build(Brightness.dark)`：

| 属性 | 值 | 说明 |
|------|------|------|
| `theme` | `AppTheme.build(Brightness.light)` | 浅色 Material 3 主题 |
| `darkTheme` | `AppTheme.build(Brightness.dark)` | 深色 Material 3 主题 |
...
- 主页面优先使用 Material 3 surface、`SectionCard` 和共享间距 / 圆角 token，统一浅色 / 深色背景、描边、阴影、图标容器和标题说明布局
