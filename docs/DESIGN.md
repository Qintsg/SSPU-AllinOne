...
提供全局学期设置，作为课表、成绩、考试、第二课堂等后续详情页的统一默认上下文。用户只选择 `YYYY-YYYY 学年 + 秋季 / 春季 / 夏季学期`，周数由软件内置官网校历按周一自动计算，不再暴露手动周数输入。内置日期定位覆盖 2023-2024 至 2026-2027 学年；2023 年以前官网已知学期保留为可选择项，但会提示暂无日期定位，供后续课程表等功能按“不可定位”状态降级。夏季学期视为一个长学期，教学段逐年内置，允许 3+2 或 2+3 等不同结构，中间非教学区间显示为暑假；其它未落在教学段或夏季长范围内的区间显示为寒假。
