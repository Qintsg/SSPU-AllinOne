...
| UI 体系 | Fluent 2 on Flutter | 项目内 `lib/design/` | Flutter 框架承载运行时，视觉层直接使用 Fluent 2 令牌、组件与响应式导航 |
...
应用在紧凑宽度使用底部 Fluent 导航，在中等与扩展宽度使用项目自建 Fluent 侧栏，在大屏宽度使用项目自建 Fluent 抽屉式侧栏，保持桌面、平板与移动端的导航层级一致。
...
- **Medium**（600–840px）：紧凑 Fluent 侧栏
- **Expanded**（840–1200px）：扩展 Fluent 侧栏
- **Large / Extra large**（>= 1200px）：Fluent 抽屉式侧栏
