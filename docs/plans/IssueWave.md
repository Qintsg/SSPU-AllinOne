...
## Idea
围绕 GitHub issue #132、#126、#127、#128、#107 建立一轮可长期演进的前端与本地安全能力完善计划。计划按优先级先处理 P0/P1 的信息中心微信推文重复 tag、公众号/服务号名称与 ID 展示、密码保护系统级快速验证，再推进 P2 的响应式布局与 Material 3 视觉统一。所有变更必须遵循 `CLAUDE.md`、仓库工作流、Flutter/Material 3 架构、本地隐私原则；本地插件/worktree 运行态配置仍需纳入 .gitignore 保护。

## Implementation
- 确认仓库工作流与本地配置边界：采用 `CLAUDE.md`、.github 分支/PR 规范和 docs 设计/使用文档作为约束来源，并检查 .gitignore 是否覆盖新增本地插件、agent 运行态、worktree、临时状态目录。
...
- 推进 #128 Material 3 视觉统一第一轮治理：梳理颜色、间距、圆角、阴影、层级、hover/active/disabled/loading/empty/error 状态，优先抽取或复用高影响的样式/组件约定。
