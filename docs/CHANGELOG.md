...
- 本版本是 `v0.4.1-beta` 预发布，聚焦校园服务能力补全、个人数据 MCP 服务、响应式页面收敛与截图反馈修正；仍建议在真实校园账号和各平台设备上继续验收后再作为稳定版本使用。
...
- 主页支持卡片/服务摘要显隐、排序和模块级停止获取配置，并与手动刷新、自动刷新及详情入口联动。
- 教务新增培养计划、空闲教室详情页；课程表整合课程与考试议程，支持标准 `.ics` 导出并交系统关联应用导入。
- 学校邮箱支持分页读取、IMAP `\Seen` 已读回写、本地搜索、收件附件按需下载，以及 SMTP 纯文本附件发送。
- 校园卡详情新增本地消费趋势统计，支持全部、近 7 天、近 30 天、自定义日期范围和日/周/月聚合。
- 快捷入口支持学习通移动端深链、OA 会话复用和内置 WebView；公众号与服务号统一使用同一刷新链路。
- 修正课程/考试 ICS：使用 `Asia/Shanghai` 时区、原始考试结束时间与 RFC 5545 UTF-8 折行；缺少完整考试时间时不再猜测事件或提醒。
- 修正信息刷新批次去重，避免分页重叠或重复来源触发重复通知。
- 修正微信单来源通知开关边界：关闭系统通知后仍会刷新并保存文章，静音仅影响通知投递。
- 设置页新增独立的普通消息通知开关与系统通知权限状态；权限明确拒绝时会清理无效的课程/考试提醒计划。
- 培养计划和空闲教室详情页补齐空态、登录失效、网络失败、重试与窄屏布局；空闲教室刷新失败时保留上一次有效结果。

### 修复

- 首页时间轴仅保留课程、考试与待办语义，修正培养方案读取状态和校园服务卡片间距。
- 重排教务身份与指标区域，按完整成绩范围统计绩点、已获学分和已读成绩，并移除旧来源、考试与成绩详情卡片。
...
- macOS 签名公证、课程/考试提醒和消息通知仍需在对应 macOS/移动端 Runner 或真实设备完成最终验收。
- 加密 WebDAV 同步、i18n 基础设施、课程/考试系统小组件和常用文档汇总页仍未实现，详见功能矩阵与 Roadmap。
