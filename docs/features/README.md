...
| 校园网 | [`network/`](network/README.md) | 网络状态检测（门禁）与网络诊断；历史 VPN/插件方案已取消 |
...
| **设计中** | 已形成需求或技术方案，但尚无可用主流程 |
| **占位** | 仅建骨架，需求待用户描述 |
| **不再计划** | 经评估明确取消；保留文档只用于记录边界与原因 |
...
- 本目录是**设计意图**的真源；代码是**落地结果**。状态徽标必须描述当前代码，不得用未来设计覆盖现状；两者出现分歧时先按代码与 GitHub Issue 的当前状态就地修正。
