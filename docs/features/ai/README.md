...
| 本地数据 MCP 暴露 | 已实现（桌面） | — | [`mcp-server.md`](mcp-server.md) |
...
- **对外暴露（MCP）**：在独立“AI 服务”栏目把本地只读快照通过 MCP 2026-07-28 Streamable HTTP 暴露给外部客户端；默认关闭、按能力授权、回环可选 API Key、局域网强制 API Key、**仅桌面**。
