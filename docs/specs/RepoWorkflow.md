...
- CI 对非草稿 PR 默认校验 PR 标题格式、分支命名、目标分支、release label 使用、GitHub 治理文件、变更 Dart 文件格式、`flutter analyze --no-fatal-infos` 与 `flutter test`；`develop -> main` 同步 PR 会跳过变更 Dart 文件格式检查，避免历史差异阻断默认分支治理同步。
